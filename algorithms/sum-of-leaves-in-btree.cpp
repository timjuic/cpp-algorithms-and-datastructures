...
int SumOfLeaves(BinaryTree<int> &tree, BinaryTree<int>::node node, int &sum) {
   cout << "iteration" << endl;
   if (tree.LeftChild(node) == tree.lambda && tree.RightChild(node) == tree.lambda) {
      cout << tree.Label(node) << endl;
      sum += tree.Label(node);
   }
   if (tree.LeftChild(node) != tree.lambda) SumOfLeaves(tree, tree.LeftChild(node), sum);
   if (tree.RightChild(node) != tree.lambda) SumOfLeaves(tree, tree.RightChild(node), sum);
}
...
int main() {
   ...
   int sum = 0;
   SumOfLeaves(tree, tree.Root(), sum);
